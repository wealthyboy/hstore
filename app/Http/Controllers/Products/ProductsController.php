...
class ProductsController extends Controller
{
    ...
    public function show(Request $request,Category $category,ProductVariation $product_variation)  { 
    	...
    	return view('products.show',compact('inventory','stock','category','related_products','attributes','product_variation','page_title'));
    }
    ...
    public function search(Request $request){
		...
		if($request->has('q')){
			$filtered_array = array_filter($filtered_array);
                $query = ProductVariation::whereHas('categories', function( $query ) use ( $filtered_array ){
                    $query->where('product_variations.name','!=',null)
                        ->orWhere('categories.name', 'like', '%' .$filtered_array['q'] . '%')
